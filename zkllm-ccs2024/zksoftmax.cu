...
// PolyAttn: global workdir for polynomial table loading
static string g_poly_workdir = "";
...
least_significant_segments(), other_segments()
{
    ...
    for (uint i = L; i < K - M; ++ i)
    {
        ...
        bool loaded_poly = false;
        if (!g_poly_workdir.empty()) {
            string poly_path = g_poly_workdir + "/polyeval_table_" + to_string(i) + ".bin";
            ifstream test_f(poly_path, ios::binary);
            if (test_f.good()) {
                test_f.close();
                uint n_words = bs[i] * 8;
                uint* host_buf = new uint[n_words];
                FILE* fp = fopen(poly_path.c_str(), "rb");
                if (fp && fread(host_buf, sizeof(uint), n_words, fp) == n_words) {
                    cudaMemcpy(mapped_values.gpu_data, host_buf, n_words * sizeof(uint), cudaMemcpyHostToDevice);
                    loaded_poly = true;
                    printf("[PolyAttn] Loaded polynomial table seg=%d size=%d\\n", i, bs[i]);
                }
                if (fp) fclose(fp);
                delete[] host_buf;
            }
        ...
        if (!loaded_poly) {
            // Original exp-based table generation
